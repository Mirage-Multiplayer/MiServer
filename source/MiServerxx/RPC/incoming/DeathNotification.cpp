...
void IRPCFunc_DeathNotification(RPCParameters* rpcParams)
{
	RakServerInterface* pRakServer = mimp::internal::server::GetCoreInstance()->getRakServer();
	mimp::internal::CPool<mimp::CPlayer>* pPlayerPool = mimp::internal::server::GetCoreInstance()->GetNetGame()->GetPlayerPool();
	...
	mimp::internal::RPC::ORPCDeathBroadcast rpc;
	rpc.wPlayerID = playerID;
	mimp::internal::server::GetCoreInstance()->BroadcastRPC(&rpc, playerID);
}